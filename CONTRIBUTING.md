...
- Architecture (New Arch / Old Arch) and, for iOS, whether the `GameController` framework is linked
...
```sh
Running "IsKeyboardConnectedExample" with {"fabric":true,"concurrentRoot":true,...}
```
...
- **iOS** — open `example/ios/IsKeyboardConnectedExample.xcworkspace` in Xcode. Source files are under `Pods > Development Pods > react-native-is-keyboard-connected`. The module depends on the `GameController` framework (linked automatically via the podspec).
- **Android** — open `example/android` in Android Studio. Source files are under `react-native-is-keyboard-connected`.
